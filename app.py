        ...
        for index, row in my_campaigns.iterrows():
            with st.expander(f"📍 {row['اسم التجمع']} - {row['اليوم والتاريخ']}"):
                ...
                with col1:
                    ...
                    st.write(f"**دورك في الحملة:** {'قائد الفريق' if row['قائد الفريق'] == st.session_state['user_name'] else 'مراقب مشارك'}")
                with col2:
                    st.write(f"**عدد المنشآت:** {row['عدد المنشآت بناءً على المسح الميداني']}")
                    if row['موقع التجمع على الخرائط']:
                        st.link_button("فتح الموقع في الخرائط", row['موقع التجمع على الخرائط'])
                st.markdown(f"**المشاركين:** {row['المراقبين المشاركين']}")
